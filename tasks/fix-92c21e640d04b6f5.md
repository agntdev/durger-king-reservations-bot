# fix-92c21e640d04b6f5 — E2T1 spec text assertions do not match multi-line reminder message

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 DKB

tests/specs/E2T1.json asserts `text: "Reminder: your Durger King reservation is coming up."` but formatReminderMessage() sends a multi-line message with booking details. The harness uses exact string equality for text fields, so the spec fails even though behavior is correct. Update the spec expected text to the full reminder message.

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-92c21e640d04b6f5.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.
