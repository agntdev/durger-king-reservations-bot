# fix-78506e8c5dbd85b6 — E2T2 spec text assertions do not match multi-line no-show notice

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 DKB

tests/specs/E2T2.json asserts `text: "No-show flagged"` but formatNoShowNotice() sends a multi-line owner alert. Harness exact-match fails. Update expected text to the full no-show notice or assert only via method without partial text.

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-78506e8c5dbd85b6.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.
