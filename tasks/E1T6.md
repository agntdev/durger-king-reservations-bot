# E1T6 — Fix booking flow error after phone number sharing

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 DKB

Investigate and resolve the error occurring when users attempt to complete a booking after sharing their phone number. Validate phone number formatting, ensure proper data handling during booking creation, and implement error boundaries to prevent flow interruption.

## Skills

**Skills:** telegram-bot-basics, nodejs, database
...
If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/E1T6.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/E1T6.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.
