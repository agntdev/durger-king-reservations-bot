...
async function promptBookingWindow(ctx: Ctx): Promise<void> {
  ...
  await ctx.reply("How many days ahead should bookings be allowed? (e.g., 30, 60, 90)");
}
...
async function finishAdminSetup(ctx: Ctx): Promise<void> {
  ...
  adminConfig.bookingWindowDays = ctx.session.adminSetupWindow ?? adminConfig.bookingWindowDays;
  await ctx.reply(
  ...
  bot.on("message:text", async (ctx, next) => {
    ...
    if (step === "admin_setup_owner_id") {
      ...
      if (!Number.isFinite(id) || id <= 0 || !Number.isInteger(id)) {
        await ctx.reply("That doesn't look like a valid Telegram ID. Please send a numeric ID.");
        return;
      }
    ...
    if (step === "admin_setup_window") {
      ...
      if (!Number.isFinite(days) || days <= 0 || !Number.isInteger(days)) {
        await ctx.reply("Please enter a valid number of days (e.g., 30, 60, 90).");
        return;
      }
